...
class IconCompiler {
  ...
  compileEntry(key, value) {
    ...
    if (!Array.isArray(match)) {
      ...
      match = this.parseMatchPattern(match)
    ...
    for (const i of match) {
      ...
      const prevIcon = output.find(def =>
        this.canMergeDefinitions(def, newIcon)
      )

      if (prevIcon) prevIcon.merge(newIcon)
      else output.push(newIcon)
  ...
  parseExtraProps(input) {
    ...
    for (const key in output) if (output[key] == null) delete output[key]
